...
<div className="mt-8 pt-6 border-t border-white/20">
              <p className="text-xs font-semibold text-brand-200 tracking-wider mb-4">Trusted by businesses across India</p>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="flex gap-2 mb-6 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
            {([["email", "Email"], ["phone", "Phone"]] as [LoginMethod, string][]).map(([m, label]) => (
              <button key={m} onClick={() => { setMethod(m); setError(""); setOtpSent(false); }} className={`flex-1 h-11 rounded-lg text-sm font-medium transition-all ${method === m ? "bg-white dark:bg-gray-700 text-brand-600 shadow-sm" : "text-gray-500 dark:text-gray-400 hover:text-gray-700"}`}>{label}</button>
            ))}
          </div>
          ...
          {method === "email" && (
            <form onSubmit={handleEmailLogin} className="space-y-5">
              <Input label="Email" type="email" placeholder="agent@company.com" value={email} onChange={(e) => setEmail(e.target.value)} icon={<Mail className="w-4 h-4" />} required className="h-11" />
              <div className="relative">
                <Input label="Password" type={showPassword ? "text" : "password"} placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} icon={<Lock className="w-4 h-4" />} required className="h-11" />
              ...
              <Button type="submit" className="w-full h-11" size="lg" isLoading={isLoading}>{!isLoading && <ArrowRight className="w-4 h-4 mr-2" />}Sign In</Button>
            </form>
          )}

          {method === "phone" && (
            <div className="space-y-5">
              <Input label="Phone Number" type="tel" placeholder="+91 9876543210" value={phone} onChange={(e) => setPhone(e.target.value)} icon={<Phone className="w-4 h-4" />} disabled={otpSent} required className="h-11" />
              {otpSent && (
                <>
                  <Input label="Enter OTP" type="text" placeholder="6-digit OTP" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} maxLength={6} required className="h-11" />
                  <Button onClick={handleVerifyOtp} className="w-full h-11" size="lg" isLoading={isLoading}><Zap className="w-4 h-4 mr-2" />Verify OTP</Button>
                  <button onClick={handleSendOtp} className="w-full text-sm text-brand-600 hover:text-brand-700 font-medium">Resend OTP</button>
                </>
              )}
              {!otpSent && (
                <Button onClick={handleSendOtp} className="w-full h-11" size="lg" isLoading={isLoading}><Phone className="w-4 h-4 mr-2" />Send OTP</Button>
              )}
            </div>
          )}

          {/* Divider */}
          <div className="flex items-center gap-4 my-6"><div className="flex-1 h-px bg-gray-200 dark:bg-gray-700" /><span className="text-xs text-gray-400">Or continue with</span><div className="flex-1 h-px bg-gray-200 dark:bg-gray-700" /></div>

          {/* Social Login Buttons */}
          <div className="space-y-3">
            <a href={`${API_URL}/auth/google`} className="w-full h-11 flex items-center justify-center gap-3 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            ...
            <a href={`${API_URL}/auth/facebook`} className="w-full h-11 flex items-center justify-center gap-3 px-4 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
...
<div className="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 text-center">
            ...
            <p className="text-center text-xs text-gray-400 dark:text-gray-500 mt-6 flex justify-center items-center gap-4 flex-wrap">
              <Link href="/help" className="inline-flex min-h-11 items-center px-2 hover:text-brand-600">Need help?</Link>
              <Link href="/feedback" className="inline-flex min-h-11 items-center px-2 hover:text-brand-600">Send feedback</Link>
              <Link href="/blog" className="inline-flex min-h-11 items-center px-2 hover:text-brand-600">Read blog</Link>
            </p>
        </div>
      </div>
    </div>
