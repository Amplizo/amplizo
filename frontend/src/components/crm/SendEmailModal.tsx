...
import { Loader2, Search, Mail, X, CheckCircle2, AlertCircle } from "lucide-react";
...
export function SendEmailModal({ onClose, onSent }: SendEmailModalProps) {
  ...
  const sendEmail = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await api.sendEmailToCustomer({
      ...
      const msg = err?.response?.data?.message?.message || err?.response?.data?.message || "Failed to send email";
      setError(msg);
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Send Email</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Send an email to a customer</p>
          </div>
        ...
        {emailConfigured === false && (
          <div className="mx-5 mt-5 p-3 rounded-xl bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-sm text-yellow-700 dark:text-yellow-400">
            ...
            Email service is not configured on the backend. Messages cannot be sent. Please configure SMTP credentials.
          </div>
        )}
        ...
        {success ? (
          <div className="p-8 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-lg font-semibold text-gray-900 dark:text-gray-100">Email Sent!</p>
            <p className="text-sm text-gray-500">The email has been sent successfully.</p>
          </div>
        ) : (
          <form onSubmit={(e) => { e.preventDefault(); sendEmail(); }} className="p-5 space-y-4">
            {error && (
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">Email from selected customer record</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
            ...
            <div className="flex gap-2 pt-2">
              ...
              <button
                type="submit"
                disabled={submitting || !selectedCustomer || !subject.trim() || !message.trim() || emailConfigured === false}
                className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#0A66FF] text-white text-sm font-semibold hover:bg-[#0952CC] disabled:opacity-60 transition-colors"
              >
